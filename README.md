...
首页每个系列只放几张代表作（瀑布流），点 View all 进入 `#/slug` 系列页才看到全部。代表作从 [js/site.js](js/site.js) 的 `HIGHLIGHTS` 候选池里**每次随机抽**（`HOME_COUNT` 张，尽量每个地点一张），并且每 5 秒会有一张在原位淡入淡出换成池子里的另一张（同为横/竖构图，标签页不可见或开着灯箱时暂停，系统开了减少动态效果则不换）。没填候选池的系列整个系列都是候选。
